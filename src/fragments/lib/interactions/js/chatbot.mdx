...
```typescript
import { Interactions, type InteractionsResponse } from '@aws-amplify/interactions';

Interactions.onComplete({
  botName: "TheBotName",
  callback: (error?: Error, completion?: InteractionsResponse) => {
     if (error) {
...
## AWS Lex V1
The Lex V1 module is located in a different path, and we strongly advise you to migrate to the Lex V2 bot.
...
```typescript
import { Interactions as InteractionsLexV1 } from '@aws-amplify/interactions/lex-v1';
import { type InteractionsResponse } from '@aws-amplify/interactions';
